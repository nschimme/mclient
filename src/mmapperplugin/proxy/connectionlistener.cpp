...
ConnectionListener::ConnectionListener(MapData* md, Mmapper2PathMachine* pm, CommandEvaluator* ce, PrespammedPath* pp, QObject *parent)
  : QTcpServer(parent)
{
  m_accept = true;
...
void ConnectionListener::incomingConnection(int socketDescriptor)
{
  if (m_accept)
  {
    ...
    Proxy *proxy = new Proxy(m_mapData, m_pathMachine, m_commandEvaluator, m_prespammedPath, socketDescriptor, m_remoteHost, m_remotePort, true, this);
    proxy->start();
  }
  else
  {
